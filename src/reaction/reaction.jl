...
function run_reaction(rxn::Reaction, reactants::Vector{Molecule}; max_products::Int=1000)
	...
	product_sets = _reaction_run_reactants_inline_properties(rxn._rdkit_rxn, py_reactants, max_products)
	return [[Molecule(_rdkit_mol=prod, valid=!pynot(prod), source="reaction") for prod in set] for set in product_sets]
end
...
function get_reactant_template(rxn::Reaction, idx::Int)
	...
	return Molecule(_rdkit_mol=template, valid=!pynot(template), source="reaction_template")
end
...
function has_reactant_substructure_match(rxn::Reaction, mol::Molecule)
	return pyconvert(Bool, _reaction_has_reactant_substructure_match(rxn._rdkit_rxn, mol._rdkit_mol))
end
...
```julia
rxn1 = reaction_from_smarts("\\[C:1\\](=O)\\[O:2\\]\\[C:3\\]>>\\[C:1\\](=O)\\[O-\\].\\[C:3\\]\\[O+\\]")
rxn2 = reaction_from_smarts("\\[C:1\\](=O)\\[O:2\\]\\[C:3\\]>>\\[C:1\\](=O)\\[OH\\].\\[C:3\\]")
...
function enumerate_library(rxn::Reaction, reactant_lists::Vector{Vector{Molecule}})
	...
	product_sets = _reaction_enumerate_library_from_reaction(rxn._rdkit_rxn, py_reactant_lists)
	return [[Molecule(_rdkit_mol=prod, valid=true, source="library") for prod in set] for set in product_sets]
end
...
"""
function find_similar_reactions(target_rxn::Reaction, reaction_db::Vector{Reaction};
							   threshold::Float64=0.7, method::Symbol=:tanimoto)
