...
function generate_3d_conformers(
    ...
    try
        ...
        mol_copy = _add_hs(mol._rdkit_mol)
        ...
        # Generate conformers
        if num_conformers == 1
            conf_id = _embed_molecule(mol_copy, params)
            conf_ids = pyconvert(Int, conf_id) == -1 ? Int[] : [pyconvert(Int, conf_id)]
        else
            try
                conf_ids = _embed_multiple_confs_with_kwargs(
                ...
                conf_ids = _embed_multiple_confs(mol_copy, num_conformers, params)
                conf_ids = [pyconvert(Int, cid) for cid in conf_ids]
            end
        end
...
function _optimize_conformer_with_mol!(
    ...
    try
        if force_field == :mmff
            ff = _mmff_get_molecule_force_field(
                mol_rdkit, _mmff_get_molecule_properties(mol_rdkit); confId = conformer_id
            )
        elseif force_field == :uff
            ff = _uff_get_molecule_force_field(mol_rdkit; confId = conformer_id)
        else
...
function _optimize_conformer!(
    ...
    try
        mol_with_hs = _add_hs(mol._rdkit_mol)

        if force_field == :mmff
            ff = _mmff_get_molecule_force_field(
                mol_with_hs,
                _mmff_get_molecule_properties(mol_with_hs);
                confId = conformer_id,
            )
        elseif force_field == :uff
            ff = _uff_get_molecule_force_field(mol_with_hs; confId = conformer_id)
        else
            error("Unsupported force field: $force_field")
        end
...
function generate_2d_conformers(mol::Molecule)
    ...
    mol_copy = _mol_copy(mol._rdkit_mol)
    _compute_2d_coords(mol_copy)
