...
"""
function brics_decompose(
    mol::Molecule;
    min_fragment_size::Int = 1,
    max_fragment_size::Union{Int, Nothing} = nothing,
)
    !mol.valid && return missing

    try
        # Get BRICS fragments
        brics_frags = _brics_decompose(mol._rdkit_mol; minFragmentSize = min_fragment_size)
...
function recap_decompose(mol::Molecule; min_fragment_size::Int = 1)
    ...
    try
        ...
        recap_tree = _recap_decompose(mol._rdkit_mol)
        ...
        for leaf in leaves.values()
            ...
            if !pynot(frag_mol)
                ...
                temp_molecule = Molecule(;
                    _rdkit_mol = frag_mol, valid = true, source = "recap_fragment"
                )
...
function get_murcko_scaffold(mol::Molecule)
    ...
    try
        scaffold_mol = _get_scaffold_for_mol(mol._rdkit_mol)

        if pynot(scaffold_mol)
            return missing
        end

        scaffold_molecule = Molecule(;
            _rdkit_mol = scaffold_mol, valid = true, source = "murcko_scaffold"
        )
...
function get_generic_scaffold(mol::Molecule)
    ...
    try
        scaffold_mol = _get_scaffold_for_mol(mol._rdkit_mol)
        generic_mol = _make_scaffold_generic(scaffold_mol)
        ...
        generic_molecule = Molecule(;
            _rdkit_mol = generic_mol, valid = true, source = "generic_scaffold"
        )
...
function fragment_by_bonds(mol::Molecule, bond_indices::Vector{Int})
    ...
    try
        ...
        fragmented = _fragment_on_bonds(mol._rdkit_mol, bond_indices)

        # Get individual fragments
        frags = _get_mol_frags(fragmented; asMols = true)

        fragments = Vector{String}()
        for frag in frags
            if !pynot(frag)
                frag_molecule = Molecule(;
                    _rdkit_mol = frag, valid = true, source = "bond_fragment"
                )
...
function get_fragment_count(mol::Molecule)
    ...
    try
        ...
        frags = _get_mol_frags(mol._rdkit_mol; asMols = false)
        return length(frags)
    catch e
...
function split_fragments(mol::Molecule)
    ...
    try
        ...
        frags = _get_mol_frags(mol._rdkit_mol; asMols = true)

        fragments = Vector{Molecule}()
        for (i, frag) in enumerate(frags)
            if !pynot(frag)
                frag_molecule = Molecule(;
                    _rdkit_mol = frag, valid = true, source = "fragment_$i"
                )
