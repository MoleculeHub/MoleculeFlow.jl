    ...
    if use_svg
        ...
        try
            ...
            try
                ...
                svg_result = _mol_to_image(mol._rdkit_mol; size = size, useSVG = true)
...
function highlight_substructure(
    ...
    return mol_to_image(
        mol; size = size, highlight_atoms = highlight_atoms, use_svg = use_svg
    )
end
...
function draw_molecule_with_atom_labels(
    ...
    if use_svg
        try
            ...
            svg_str = _mols_to_grid_image(
                [mol._rdkit_mol]; molsPerRow = 1, subImgSize = size, useSVG = true
            )
            return pyconvert(String, svg_str)
        catch e
            @warn "SVG generation failed for atom labels, falling back to PNG: $e"
            # Fall back to PNG
            img = _mol_to_image(mol._rdkit_mol; size = size)
            return pil_png_to_rgb(img)
        end
    else
        img = _mol_to_image(mol._rdkit_mol; size = size)
